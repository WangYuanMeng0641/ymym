...
# 改进的SHAP分析函数 - 直接分析所有1120个特征，找出每个亚细胞的前10个重要特征（不重复）
def perform_shap_analysis_all_features(model, dataloader, label_columns, device, output_path):
    """
    使用PermutationExplainer直接分析所有1120个特征
    为每个亚细胞找出前10个最重要的特征（不重复）
    """
    print("\n" + "=" * 80)
    print("开始全特征SHAP分析（1120个特征）")
    print("=" * 80)
    ...
    # 收集所有数据
    with torch.no_grad():
        for cksnap, kmer_raw, adj, labels in tqdm(dataloader, desc="收集数据"):
            all_cksnap.append(cksnap.numpy())
            all_kmer_raw.append(kmer_raw.numpy())

            kmer_raw_tensor = kmer_raw.to(device)
            kmer_processed = model.mlp_kmer(kmer_raw_tensor).cpu().numpy()
            all_kmer_processed.append(kmer_processed)

            adj_tensor = adj.to(device)
            batch_size = adj_tensor.size(0)
            node_features = model.create_fixed_node_features().unsqueeze(0).repeat(batch_size, 1, 1).to(device)
            graph_processed = model.graph_processor(node_features, adj_tensor).cpu().numpy()
            all_graph.append(graph_processed)

            all_labels.append(labels.numpy())

    # 合并数据
    X_cksnap = np.vstack(all_cksnap)
    X_kmer_raw = np.vstack(all_kmer_raw)
    X_kmer_processed = np.vstack(all_kmer_processed)
    X_graph = np.vstack(all_graph)
    y = np.vstack(all_labels)

    # 拼接成1120维特征
    X_combined = np.hstack([X_cksnap, X_kmer_processed, X_graph])

    print(f"数据形状: 合并后={X_combined.shape}, 标签={y.shape}")
    ...
    for g in range(6):
        for pair in aaPairs:
            feature_names.append(f"CKSNAP_gap{g}_{pair}")

    # 2. Kmer特征名称 (256维) - 改进的映射方法
    if hasattr(model, 'kmer_names') and len(model.kmer_names) >= 5460:
        print("正在分析Kmer MLP层的权重映射...")
        ...
        if len(linear_layers) >= 2:
            w1 = linear_layers[0].weight.detach().cpu().numpy()  # [1024, 5460]
            w2 = linear_layers[1].weight.detach().cpu().numpy()  # [256, 1024]

            # 计算每个256维输出对原始k-mer的综合重要性
            # 使用绝对值的乘积作为重要性度量
            combined_importance = np.zeros((256, 5460))

            for i in range(256):
                # 第i个输出神经元对所有1024个中间神经元的权重
                w2_i = w2[i, :]  # [1024]

                # 计算通过每个中间神经元的路径重要性
                for j in range(1024):
                    importance = abs(w2_i[j])
                    if importance > 0.01:  # 只考虑重要连接
                        combined_importance[i, :] += importance * abs(w1[j, :])

            # 为每个256维输出生成名称
            for i in range(256):
                # 找出最重要的原始k-mer
                top_indices = np.argsort(combined_importance[i])[-3:]  # 取前3个
                top_kmers = [model.kmer_names[idx] for idx in top_indices if idx < len(model.kmer_names)]

                if top_kmers:
                    # 使用最重要的k-mer作为特征名称
                    feature_names.append(f"Kmer_{top_kmers[-1]}_out{i}")
                else:
                    feature_names.append(f"Kmer_feat_{i}")
        else:
            feature_names.extend([f"Kmer_feat_{i}" for i in range(256)])
    else:
        feature_names.extend([f"Kmer_feat_{i}" for i in range(256)])
    ...
    for i, kmer in enumerate(kmers_3):
        for j, feat_type in enumerate(feature_types):
            feature_names.append(f"Graph_{kmer}_{feat_type}")

    print(f"总特征数: {len(feature_names)}")

    # 存储所有细胞类型的结果，用于后续对比分析
    all_top_features = {}

    # 对每个亚细胞定位进行分析
    for cell_idx, cell_type in enumerate(label_columns):
        print(f"\n分析亚细胞: {cell_type}")
        ...
        print(f"  总样本数: {len(y_cell)}, 正样本: {pos_count} ({pos_count / len(y_cell) * 100:.1f}%)")

        # 采样进行SHAP分析
        sample_size = min(300, X_combined.shape[0])
        indices = np.random.choice(X_combined.shape[0], sample_size, replace=False)
        ...
        try:
            ...
            explainer = PermutationExplainer(
                ...
                background,
                max_evals=max(4000, 2 * X_sample.shape[1] + 1)
            )
            ...
            for idx in sorted_indices:
                if len(top10_indices) >= 10:
                    break

                feat_name = feature_names[idx]

                # 检查是否重复
                # 对于k-mer特征，检查k-mer序列是否已经出现过
                is_duplicate = False
                if feat_name.startswith('Kmer_'):
                    ...
                    if len(parts) >= 2:
                        ...
                        for selected in selected_features:
                            if selected.startswith('Kmer_') and kmer_seq in selected:
                                is_duplicate = True
                                break
                elif feat_name in selected_features:
                    is_duplicate = True

                if not is_duplicate:
                    top10_indices.append(idx)
                    top10_features.append(feat_name)
                    top10_shap_values.append(mean_abs_shap[idx])
                    selected_features.add(feat_name)

            # 存储结果
            all_top_features[cell_type] = {
                'indices': top10_indices,
                'features': top10_features,
                'shap_values': top10_shap_values,
                'shap_values_array': shap_values_array[:, top10_indices],
                'feature_values': X_sample[:, top10_indices]
            }

            # 保存到CSV
            importance_df = pd.DataFrame({
                'Feature_Index': top10_indices,
                'Feature_Name': top10_features,
                'Mean_Abs_SHAP': top10_shap_values,
                'Feature_Type': [get_feature_type(idx) for idx in top10_indices]
            })

            importance_file = os.path.join(output_path, f'top10_features_{cell_type}.csv')
            importance_df.to_csv(importance_file, index=False)
            print(f"  前10个特征已保存到: {importance_file}")

            # 打印前5个最重要特征
            print("  前5个最重要特征:")
            for i in range(min(5, len(top10_features))):
                print(f"    {i + 1}. {top10_features[i]} (SHAP={top10_shap_values[i]:.6f})")
            ...
            if len(top10_indices) > 0:
                ...
                for name in top10_features:
                    if name.startswith('Kmer_'):
                        parts = name.split('_')
                        if len(parts) >= 2:
                            short_names.append(f"kmer_{parts[1]}")
                        else:
                ...
                # 绘制SHAP摘要图
                shap.summary_plot(
                    shap_values_array[:, top10_indices],
                    X_sample[:, top10_indices],
                ...
                plt.title(f"{cell_type} - Top 10 Important Features", fontsize=14, fontweight='bold')
                plt.tight_layout()

                plot_file = os.path.join(output_path, f'top10_shap_summary_{cell_type}.png')
                plt.savefig(plot_file, dpi=300, bbox_inches='tight')
                plt.close()
                print(f"  SHAP摘要图已保存到: {plot_file}")

        except Exception as e:
            print(f"  SHAP分析失败: {e}")
            print("  尝试使用基于模型权重的特征重要性分析...")
            analyze_feature_importance_by_weights_all(model, feature_names, cell_type, output_path, device)

    # 分析特征类型分布
    analyze_feature_type_distribution(all_top_features, output_path)

    print("\n全特征SHAP分析完成！")


def analyze_feature_type_distribution(all_top_features, output_path):
    """分析每个亚细胞的前10个特征中，不同类型的分布"""
    print("\n分析特征类型分布...")

    # 收集所有细胞类型的特征类型
    feature_types_by_cell = {}
    for cell_type, data in all_top_features.items():
        indices = data['indices']
        types = [get_feature_type(idx) for idx in indices]
        feature_types_by_cell[cell_type] = types

    # 计算每个细胞类型中不同特征类型的数量
    type_counts_by_cell = {}
    for cell_type, types in feature_types_by_cell.items():
        type_counts = {
            'CKSNAP': types.count('CKSNAP'),
            'Kmer': types.count('Kmer'),
            'Graph': types.count('Graph')
        }
        type_counts_by_cell[cell_type] = type_counts

    # 创建DataFrame用于显示
    df_counts = pd.DataFrame(type_counts_by_cell).T
    print("\n各亚细胞前10个特征中不同类型特征的数量:")
    print(df_counts)

    # 绘制堆叠条形图
    plt.figure(figsize=(12, 8))
    df_counts.plot(kind='bar', stacked=True, colormap='tab20c')
    plt.title('Distribution of Feature Types in Top 10 Features', fontsize=16, fontweight='bold')
    ...
    # 保存图像
    plot_file = os.path.join(output_path, 'feature_type_distribution.png')
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"特征类型分布图已保存到: {plot_file}")

    # 计算总体分布
    total_counts = {
        'CKSNAP': df_counts['CKSNAP'].sum(),
        'Kmer': df_counts['Kmer'].sum(),
        'Graph': df_counts['Graph'].sum()
    }
    print(f"\n所有亚细胞前10个特征的总体分布:")
    for feat_type, count in total_counts.items():
        percentage = count / (len(all_top_features) * 10) * 100
        print(f"  {feat_type}: {count} ({percentage:.1f}%)")


def analyze_feature_importance_by_weights_all(model, feature_names, cell_type, output_path, device):
    """基于模型权重的特征重要性分析（全特征）"""
    ...
    if len(classifier_weights) >= 3:
        ...
        if cell_idx is not None and cell_idx < final_weights.shape[0]:
            ...
            # 选择前10个最重要的特征
            top10_indices = np.argsort(importance_1120)[-10:][::-1]
            top10_features = [feature_names[i] for i in top10_indices]
            top10_importance = importance_1120[top10_indices]

            # 归一化重要性
            if np.sum(top10_importance) > 0:
                top10_importance = top10_importance / np.sum(top10_importance)

            # 保存特征重要性结果
            importance_df = pd.DataFrame({
                'Feature_Index': top10_indices,
                'Feature_Name': top10_features,
                'Importance': top10_importance,
                'Feature_Type': [get_feature_type(idx) for idx in top10_indices]
            })

            importance_file = os.path.join(output_path, f'weight_importance_{cell_type}.csv')
            importance_df.to_csv(importance_file, index=False)
            print(f"    基于权重的特征重要性已保存到: {importance_file}")

            # 绘制条形图
            create_feature_importance_plot_all(top10_features, top10_importance,
                                               cell_type, output_path)
        else:
            print(f"  错误: 无法找到细胞类型 {cell_type} 的索引")
    else:
        print("  错误: 无法获取模型权重")
...
def get_cell_type_index(cell_type):
    ...
    return cell_type_mapping.get(cell_type, 0)  # 默认返回0


def create_feature_importance_plot_all(feature_names, importance, cell_type, output_path):
    """创建特征重要性条形图（全特征）"""
    if len(feature_names) > 0:
        ...
        bars = plt.barh(range(len(feature_names)), importance, color=colors)

        plt.xlabel('特征重要性', fontsize=12)
        plt.ylabel('特征', fontsize=12)
        plt.title(f"{cell_type} - Top 10 特征重要性（基于模型权重）", fontsize=14, fontweight='bold')
        ...
        legend_elements = [
            Patch(facecolor='#1f77b4', label='CKSNAP特征'),
            Patch(facecolor='#ff7f0e', label='Kmer特征'),
            Patch(facecolor='#2ca02c', label='Graph特征')
        ]
        ...
        # 保存图像
        plot_file = os.path.join(output_path, f'feature_importance_{cell_type}.png')
        plt.savefig(plot_file, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"    特征重要性图已保存到: {plot_file}")


# 独立测试函数，只进行三个特征一起的SHAP分析
def independent_test_with_shap(opt):
    ...
    # 进行全特征SHAP分析（三个特征一起分析）
    shap_output_path = os.path.join(CACHE_DIR, 'shap_analysis_all_features')
    os.makedirs(shap_output_path, exist_ok=True)

    print(f"\n全特征SHAP分析结果将保存到: {shap_output_path}")

    # 进行全特征SHAP分析
    print("\n" + "=" * 80)
    print("开始全特征SHAP分析（1120个特征）")
    print("=" * 80)
    perform_shap_analysis_all_features(model, ind_loader, label_columns, opt.device, shap_output_path)
